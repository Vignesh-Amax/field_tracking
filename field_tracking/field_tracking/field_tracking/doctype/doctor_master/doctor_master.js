...
async function geocode_address(frm) {
    ...
    try {
        ...
        if (data.status === "OK" && data.results.length > 0) {
            ...
            // Only update if values are found and different
            let update_needed = false;
            let update_obj = {};

            if (pincode && frm.doc.pincode !== pincode) {
                update_obj.pincode = pincode;
                update_needed = true;
            }
            if (lat && frm.doc.latitude !== String(lat)) {
                update_obj.latitude = String(lat);
                update_needed = true;
            }
            if (lng && frm.doc.longitude !== String(lng)) {
                update_obj.longitude = String(lng);
                update_needed = true;
            }
